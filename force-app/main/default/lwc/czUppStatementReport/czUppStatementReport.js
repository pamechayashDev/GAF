...
export default class CzUppStatementReport extends LightningElement {
  filters = {};
  ...
  connectedCallback() {
    ...
    fetchStatementWithDocuments({
      ...
      this.statementsFetched = response.uppStatements;
      ...
      if (results.length != 0) {
        ...
        this.filters.location = this.locationOptions[0].value;
        valuesSet.clear();
        this.statementDateOptions = this.statementsFetched
          .filter((result) => {
            if (
              !valuesSet.has(result.Statement_date__c) &&
              result.Location__c == this.filters.location
            ) {
              return true;
            }
            return false;
  ...
  downloadCSV() {
    if (!this.documents || this.documents.length === 0) {
      this.showNotification("Error", "No documents to download", "error");
      return;
    }
  ...
  handleApplyFilters() {
    ...
    fetchStatementWithDocuments({
      ...
      .catch((error) => {
        ...
        this.showNotification("Error", "error in fetching documents", "error");
        this.documents = [];
      });
  }
  ...
  handleDownloadDoc() {
    if (this.downloadDocumentLink != null) {
      ...
      this.showNotification("Error", "No document link found", "error");
    }
  }
